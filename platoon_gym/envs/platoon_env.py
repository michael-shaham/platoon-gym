...
class PlatoonEnv(gym.Env):
    ...
    metadata = {"render_modes": ["plot"], "render_fps": 10, "record": False}
    ...
    def __init__(
        ...
        """
        super().__init__()
        assert env_args["headway"].upper() in HEADWAY_OPTIONS
        ...
        if "render_fps" in env_args:
            self.metadata["render_fps"] = env_args["render fps"]
    ...
    def _get_obs(
        self, actions: Optional[List[np.ndarray]] = None
    ) -> Tuple[np.ndarray, ...]:
        """
        Args:
            actions: List[np.ndarray], the action of each
        ...
        self._update_history(observations, errors, actions)
    ...
    def reset(
        ...
        self.act_history = [np.array([]) for _ in range(self.n_veh)]
    ...
    def _init_render(self):
        ...
        default_size = (6, 4) if self.vehs[0].n == 2 else (8, 4)
        self.plot_size = (
            self.env_args["plot size"] if "plot size" in self.env_args else default_size
        )

        if "render dpi" in self.env_args:
    ...
    def _remove_render_lines(self):
        ...
        for k, i in enumerate(self.plot_inds):
            ...
            if self.vehs[0].n == 3:
                ...
                self.accel_lines[k] = self.accel_lines[k].pop(0)
                self.accel_lines[k].remove()
                self.accel_lines[k] = self.ax[1, 2].plot(
                    self.time_history, self.state_history[i][2, :], color=f"C{k}"
                )
    ...
    def _set_ax_lims(self):
        ...
        if self.vehs[0].n == 3:
            action_lims = (
                min([self.act_history[i][0, :].min() for i in range(self.n_veh)]) - 0.1,
                max([self.act_history[i][0, :].max() for i in range(self.n_veh)]) + 0.1,
            )
            accel_lims = (
                min([self.state_history[i][2, :].min() for i in range(self.n_veh)])
                - 0.1,
                max([self.state_history[i][2, :].max() for i in range(self.n_veh)])
                + 0.1,
            )
    ...
    def _update_history(
        ...
        if actions is None:
            actions = [np.zeros((v.dyn.m, 1)) for v in self.vehs]
        for i, (obs, err, actions) in enumerate(zip(observations, errors, actions)):
            ...
            if self.act_history[i].shape[1] > self.n_plot:
