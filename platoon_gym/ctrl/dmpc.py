...
class DMPC(ControllerBase):
    ...
    def control(
        ...
        if "optimal" not in self.prob.status:
            ...
            # fig.suptitle(f'x0={np.round(x0, 2)}, xf={np.round(xf, 2)}')
            # for i in range(len(y_neighbors)):
            #     ax[0].plot(y_neighbors[i][:, 0], label=f"neighbor {i}")
            #     ax[1].plot(y_neighbors[i][:, 1], label=f"neighbor {i}")
            # ax[0].plot(xa[:, 0], label="assumed")
            # ax[1].plot(xa[:, 1], label="assumed")
            # for a in ax:
            #     a.legend()
            #     a.grid()
            # plt.show()
