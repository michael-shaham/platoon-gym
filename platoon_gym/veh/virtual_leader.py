...
class VirtualLeader:
    ...
    def init_traj(self):
        ...
        elif self.traj_type == "velocity trajectory":
            assert "velocity trajectory" in self.traj_args
            self.velocity_trajectory = self.traj_args["velocity trajectory"]
            self.state[0] = 0.0
    ...
    def step(self) -> bool:
        ...
        """
        self.time = round(self.time + self.dt, 1)
        self.timestep += 1
        if self.traj_type == "constant velocity":
