...
def evaluate_dmpc(
    ...
    if topology.lower() == "pf":
        ...
        qneighbors = [[qself[i] / 2.0, qself[i] / 2.0] for i in range(N - 1)] + [
            [qself[-1]]
        ]
    ctrls = [
        create_controller(
    ...
    for _ in tqdm(range(round(total_time / dt))):
        ...
        for i in range(len(obs)):
            ...
            if i == 0:
                ...
                end_state[0] -= time_headways[i][0] * end_state[1]
    ...
    for spacing_policy in spacing_policies:
        for topology in topologies:
            for output_norm in norms:
                ...
                np.save(
                    data_file + "_predecessor_error_data.npy", predecessor_error_data
                )
