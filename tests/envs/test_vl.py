...
def test_constant_velocity():
    ...
    vl_plan = env_info["virtual_leader_plan"][:, : H + 1]
    actions = []
...
def test_random_step_no_accel():
    ...
    vl_traj_args = {
        ...
        "step_vel_min": step_vel_min,
        "step_vel_max": step_vel_max,
        "seed": seed,
    }
    vl = VirtualLeader(vl_traj_type, vl_traj_args, velocity=vl_vel)
...
def test_random_step_with_accel():
    ...
    vl_traj_args = {
        ...
        "step_vel_min": step_vel_min,
        "step_vel_max": step_vel_max,
        "step_accel_min": step_acc_min,
        "step_accel_max": step_acc_max,
        "seed": seed,
    }
    vl = VirtualLeader(vl_traj_type, vl_traj_args, velocity=vl_vel)
...
def test_i24_lfbk():
    ...
    render_mode = "human"
    config = {"virtual_leader": vl, "dt": dt, "render_fps": 10}
    env = gym.make("platoon_env-v0", config=config, render_mode=render_mode)
    obs, env_info = env.reset()
    ...
    while True:
        ...
        actions = []
        for i, o in enumerate(obs):
            dh = env.unwrapped.distance_headway[i]
            th = env.unwrapped.time_headway[i]
            distance_error = o[0] - dh - veh_states[i][1] * th
            velocity_error = o[1]
            error = np.array([distance_error, velocity_error])
            action, _ = ctrl.control(error)
            action = np.clip(action, -3.0, 3.0)
            actions.append(action)
        obs, _, term, trunc, env_info = env.step(action=actions)
        env.render()
        if trunc or term:
            break
