...
def test_constant_velocity():
    ...
    plot_vl_plan(vl, "constant_velocity")
...
def test_i24():
    ...
    vl.reset()
    time_forecast = vl.dt * np.arange(0, vl.i24_trajectory.shape[1])
    fig, ax = plt.subplots(2, 1, figsize=(4, 4), sharex=True)
    fig.suptitle("i24 Virtual Leader")
    fig.subplots_adjust(0.20, 0.13, 0.9, 0.9, 0.2, 0.2)
    ax[0].plot(time_forecast, vl.i24_trajectory[0, :])
    ax[0].set_ylabel("position")
    ax[1].plot(time_forecast, vl.i24_trajectory[1, :])
    ax[1].set_ylabel("velocity")
